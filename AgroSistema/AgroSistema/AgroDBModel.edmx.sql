...
-- --------------------------------------------------
-- Date Created: 08/10/2013 21:39:30
-- Generated from EDMX file: C:\Users\Win7VM\Documents\Projeto_Agro\AgroSistema\AgroSistema\AgroSistema\AgroDBModel.edmx
-- --------------------------------------------------
...
GO
IF OBJECT_ID(N'[dbo].[FK_FazendaFuncionario]', 'F') IS NOT NULL
    ALTER TABLE [dbo].[Funcionarios] DROP CONSTRAINT [FK_FazendaFuncionario];
GO
...
CREATE TABLE [dbo].[Funcionarios] (
    ...
    [Data_Demissao] nvarchar(max)  NULL,
    [Telefone] nvarchar(max)  NULL,
...
-- Creating foreign key on [Fazenda_Id] in table 'Funcionarios'
ALTER TABLE [dbo].[Funcionarios]
ADD CONSTRAINT [FK_FazendaFuncionario]
    FOREIGN KEY ([Fazenda_Id])
    REFERENCES [dbo].[Fazendas]
        ([Id])
    ON DELETE NO ACTION ON UPDATE NO ACTION;

-- Creating non-clustered index for FOREIGN KEY 'FK_FazendaFuncionario'
CREATE INDEX [IX_FK_FazendaFuncionario]
ON [dbo].[Funcionarios]
    ([Fazenda_Id]);
GO
